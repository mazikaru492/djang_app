...
def index(request):
    ...
    if request.method == 'POST':
        ...
        if request.POST['mode'] == '__check_form__':
            ...
            for item in request.POST.getlist('groups'):
                glist.append(item)
            # Messageの取得
            contents = get_your_group_message(request.user, \
                    glist, None)
        ...
        if request.POST['mode'] == '__search_form__':
            ...
            for item in gps:
                glist.append(item)
            # メッセージを取得
            contents = get_your_group_message(request.user, glist, \
                    request.POST['search'])
    ...
    # GETアクセス時の処理
        ...
        for item in gps:
            glist.append(item)
        # メッセージの取得
        contents = get_your_group_message(request.user, glist, None)
    # 共通処理
    params = {
...
def groups(request):
    ...
    if request.method == 'POST':
        ...
        if request.POST['mode'] == '__friends_form__':
            ...
            # UserのリストにふくまれるユーザーがとうろくしたFriendを取得
            fds = Friend.objects.filter(owner=request.user) \
...
def add(request):
    ...
    frd.save()
    # メッセージを設定
    messages.success(request, add_user.username + \
        ' を追加しました！groupページに移動して、追加したFriendをメンバーに設定して下さい。')
    return redirect(to='/sns')
...
def get_your_group_message(owner, glist, find):
    ...
    if find == None:
        result = Message.objects.filter(Q(group__in=groups) \
